...
def UpgradeEmmc(Upgrade, ser, DUTip, TFTPip, dlpEth, NORimage, BOOTfile, ROOTfile, emmcImage):
        ...
        while True:
            ...
            while ser.inWaiting():
                ...
                if ("Hit any key to stop autoboot:") in response != -1:
                    ...
                    ser.send_and_result(ser, (("tftp -gr %s %s\n") % (BOOTfile, TFTPip)), "root@TinyLinux:~#", "root@TinyLinux:~#")
                    sleep(1)
                    ser.send_and_result(ser, (("tftp -gr %s %s\n") % (ROOTfile, TFTPip)), "root@TinyLinux:~#", "root@TinyLinux:~#")
                    sleep(1)
                    ser.send_and_result(ser, (("tftp -gr %s %s\n") % (emmcImage, TFTPip)), "root@TinyLinux:~#", "root@TinyLinux:~#")
                    sleep(1)
                    ser.send_and_result(ser, (("tftp -gr flex-installer %s\n") % TFTPip), "root@TinyLinux:~#", "root@TinyLinux:~#")
                    sleep(1)
                    ...
                    if ser.send_and_result(ser, "reboot\n", "localhost login:", "localhost login:") != -1:
                        ...
                        Upgrade.logger.info("\n")
                        Upgrade.logger.info("######## EMMC Image Upgrade Failed ########\n")
